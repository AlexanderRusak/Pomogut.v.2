$(document).on('ready', function() {
  ...
  jQuery(function($) {
    function fix_size() {
      ...
      function setsize() {
        var img = $(this),
          img_dom = img.get(0),
          container = img.parents('.block-1 .col .block-1-img img');
        if (img_dom.complete) {
          resize();
  ...
  jQuery(function($) {
    function fix_size() {
      var images = $('.header .row div:nth-child(1) .logo-for-fixed img');
      images.each(setsize);

      function setsize() {
        var img = $(this),
          img_dom = img.get(0),
          container = img.parents('.header .row div:nth-child(1) .logo-for-fixed)');
        if (img_dom.complete) {
          resize();
        } else img.one('load', resize);
